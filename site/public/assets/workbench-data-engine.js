(() => {
  ...
  const filterSortRows = (rows, state) => {
    ...
    const sorts=Array.isArray(state.sorts) && state.sorts.length
      ? state.sorts.filter(item=>item?.key).map(item=>({key:item.key,dir:item.dir==='desc'?'desc':'asc'}))
      : (state.sortKey ? [{key:state.sortKey,dir:state.sortDir==='desc'?'desc':'asc'}] : []);
    if(sorts.length){
      out=[...out].sort((a,b)=>{
        for(const spec of sorts){
          const av=cellValue(a,spec.key),bv=cellValue(b,spec.key);
          if(av==null && bv!=null) return 1;
          if(av!=null && bv==null) return -1;
          let cmp=0;
          if(typeof av==='number' && typeof bv==='number') cmp=av-bv;
          else cmp=String(av ?? '').localeCompare(String(bv ?? ''),undefined,{numeric:true,sensitivity:'base'});
          if(cmp) return spec.dir==='desc' ? -cmp : cmp;
        }
  ...
  const performanceFixture = ({count=500,iterations=60}={}) => {
    ...
    for(let i=0;i<iterations;i++){
      const state={
        ...
        sorts:[
          {key:i%2?'company':'capex',dir:i%3===0?'desc':'asc'},
          {key:'target_date',dir:'desc'},
        ],
      };
