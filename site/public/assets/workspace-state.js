(() => {
  ...
  const create = ({columns = [], defaultSort = '', defaultDir = 'asc'} = {}) => {
    ...
    const state = {
      ...
      widgets:{
        ...
        layout:'default',
        sizes:{chartHeight:170,inspectorWidth:340},
      },
  ...
  const restore = (state, saved, {columns=[]}={}) => {
    ...
    const restoredSorts=Array.isArray(table.sorts)
      ? table.sorts.filter(item=>item && available.has(item.key)).map(item=>({key:item.key,dir:item.dir==='desc'?'desc':'asc'}))
      : [];
    const fallbackSortKey=String(table.sortKey || '');
    const fallbackSortDir=table.sortDir==='desc'?'desc':'asc';
    if(!restoredSorts.length && fallbackSortKey && available.has(fallbackSortKey)) restoredSorts.push({key:fallbackSortKey,dir:fallbackSortDir});
    state.table={
      sortKey:restoredSorts[0]?.key || fallbackSortKey,
      sortDir:restoredSorts[0]?.dir || fallbackSortDir,
      sorts:restoredSorts,
    ...
    state.widgets={
      ...
      sizes:{chartHeight:170,inspectorWidth:340,...(widgets.sizes||{})},
    };
