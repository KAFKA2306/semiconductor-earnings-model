...
const KAFKA_SIGNAL_RELEASE = 'kafka-signal-v1.0.0';
const KAFKA_SIGNAL_COMMIT = '6cceef70ab3ad5d15bad972aab0b23d954fc1fd8';
...
function addCompany(company, fallbackPeer = 'semiconductor') {
  ...
  const record = { id, ticker: normalizedTicker(company) || existing.ticker || id, name: normalizedName(company) || existing.name || id, peer };
  ...
  for (const quarter of company.quarters || []) { addPeriod(quarter.period_end); addPeriod(quarter.filing_date); }
...
const context = {
  ...
  evidence: {
    earnings: [
      { label: '会社別一次fact', href: '#companies' }, { label: '需要側一次fact', href: '#demand' }, { label: '半導体利益fact', href: '#profit' },
      { label: 'Primary API v1', href: 'api/v1/index.json' }, { label: 'Profit API v1', href: 'api/v1/semiconductor-profit/index.json' }, { label: 'Demand API v1', href: 'api/v1/demand/index.json' },
    ],
    resilience: [
      { label: 'データ品質', href: '#quality' }, { label: '報告実績', href: '#actuals' }, { label: '同業比較', href: '#peers' }, { label: '下振れシナリオ', href: '#scenario' }, { label: '証拠オントロジー', href: '#ontology' }, { label: 'Research API v2', href: 'api/v2/semiconductor-research/index.json' },
    ],
    model: [
      { label: '観測入力', href: '#inputs' }, { label: '集計と比較', href: '#aggregate' }, { label: '派生式', href: '#formula' }, { label: '未知変数', href: '#forward' }, { label: 'Demand API v1', href: 'api/v1/demand/index.json' }, { label: 'Profit API v1', href: 'api/v1/semiconductor-profit/index.json' },
    ],
  },
};
...
function enhancePage(page) {
  ...
  const kafkaCss = '<link rel="stylesheet" href="../assets/kafka-signal.css">';
  const script = '<script src="../assets/research-context.js" defer></script>';
  const kafkaScript = '<script src="../assets/kafka-signal.js" defer></script>';
  const releaseMeta = `<meta name="kafka-signal-release" content="${KAFKA_SIGNAL_RELEASE}"><meta name="kafka-signal-commit" content="${KAFKA_SIGNAL_COMMIT}">`;
  ...
  if (!html.includes(kafkaCss)) html = html.replace('</head>', `  ${kafkaCss}\n  ${releaseMeta}\n</head>`);
  ...
  else if (!html.includes('kafka-signal.js')) html = html.replace('</body>', `  ${kafkaScript}\n</body>`);
  fs.writeFileSync(file, html);

  for (const marker of (css, kafkaCss, 'id="research-context-config"', 'research-context.js', 'kafka-signal.js', `data-research-page="${page}"`)) {
...
const manifest = {
  ...
  kafka_signal: { release: KAFKA_SIGNAL_RELEASE, commit: KAFKA_SIGNAL_COMMIT },
};
fs.writeFileSync(path.join(dist, 'research-context-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`);
console.log(`research_context=PASS companies=${manifest.company_count} periods=${manifest.period_count} pages=${manifest.pages.length} kafka_signal=${KAFKA_SIGNAL_RELEASE}`);
