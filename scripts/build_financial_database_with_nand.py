...
MICRON_BU_MARGIN=ROOT/"data/financial_db/micron_business_unit_operating_margin_fy2026.json"
...
def main():
    primary=base.load(base.PRIMARY_PATH); research=base.load(base.RESEARCH_PATH); catalog=base.load(base.CATALOG_PATH); manual=base.load(base.MANUAL_PATH); micron_bu=base.load(MICRON_BU); micron_bu_margin=base.load(MICRON_BU_MARGIN); industry=base.load(INDUSTRY); ledger=base.load(NAND); state=base.load(STATE) if STATE.exists() else {"schema_version":"nand-kpi-collection-state.v1","generated_at":None,"issues":[]}
    entities=extend_entities(base.build_entities(primary,research),industry); ids={x["id"] for x in entities}
    raw=base.research_observations(research)+base.primary_observations(primary)+base.manual_observations(manual,catalog,ids)+base.manual_observations(micron_bu,catalog,ids)+base.manual_observations(micron_bu_margin,catalog,ids)+base.manual_observations(ledger,catalog,ids)
