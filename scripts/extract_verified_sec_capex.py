...
def extract_event_capex(event: dict[str, Any], payload: dict[str, Any]) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    ...
    if not isinstance(concept, dict):
        return None, [
            {"code": "NO_STANDARD_CAPEX_CONCEPT", "event_id": event_id, "accession_number": accession}
        ]
    ...
    if not candidates:
        return None, [
            {"code": "NO_VERIFIED_STANDARD_CAPEX_FACT", "event_id": event_id, "accession_number": accession}
        ]
    ...
    chosen = max(candidates, key=lambda item: str(item.get("start") or ""))
    metric = {
        "event_id": event_id,
        ...
        "definition": "Cash payments to acquire property, plant and equipment; no inferred or company-defined CapEx values are persisted.",
        "value": chosen["val"],
        "unit": "USD",
        "taxonomy": "us-gaap",
        ...
        "verification": "exact accession + exact form + exact period end + USD + us-gaap PaymentsToAcquirePropertyPlantAndEquipment",
    }
    return metric, []
...
def audit(events: list[dict[str, Any]], payloads_by_cik: dict[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "contract": "Only SEC us-gaap PaymentsToAcquirePropertyPlantAndEquipment facts matching exact accession, form, period end and USD are persisted. Missing or conflicting facts fail closed; inferred/company-defined CapEx is never saved.",
    }
